...
import { OUT, ISSUER, AGENT, SCHEMA_URL, SCHEMA_URL_V22, SCHEMA_URL_V24, MERCHANT_ADDR, OTHER_ADDR, USDC_CONTRACT, USDC, USDT_TRC20, TRON_MERCHANT, TRON_OTHER } from './fixtures/consts.mjs';
...
function makeConfig(cred, { auditLog, statusList = 'status-clean.json', didPath, wallets, crossRailLedgerPath } = {}) {
  return {
    ...
    policy: {
      ...
      issuerDid: ISSUER, schemaAllowlist: [SCHEMA_URL, SCHEMA_URL_V22, SCHEMA_URL_V24], agentDid: AGENT,
...
// transfer
await allow('v2.4 credentialSchema accepted: transfer USDC 50 -> merchant', async () => (await governed(makeConfig('wdk-usdc-v24'))).proxy.transfer({ token: USDC_CONTRACT, recipient: MERCHANT_ADDR, amount: USDC(50) }));
